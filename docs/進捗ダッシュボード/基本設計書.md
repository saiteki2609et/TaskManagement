...
- 機能行を展開すると表示される成果物行も、同じ機能の中(同一階層内)であればドラッグ&ドロップで並び替えられる。`reorderDeliverablesAction`で`Deliverable.order`を更新する(6.4節)。機能をまたいだ並び替えはできない(Todoの同一階層内並び替えと同じ制約)。この対応にあわせて`Deliverable.order`のスコープを「機能×工程」から「機能」単位に変更した(6.4節・詳細設計書 0.2節)。
...
| `reorderDeliverablesAction(projectId, featureId, orderedIds)` | ガントビューの成果物行ドラッグ&ドロップ並び替え用(4.2節)。指定した機能配下の成果物に限定して`orderedIds`の並び順どおりに`order`を0始まりで振り直す |

`Deliverable.order`のスコープ変更: 当初は「機能×工程」単位(セル単位)で管理していたが、ガントビューの成果物行並び替え(機能単位でまとめて表示するため工程をまたいで並べ替える必要がある)に合わせて「機能」単位に変更した。マトリクス表のセル表示は特定の(機能,工程)の組み合わせで絞り込むだけなので、スコープ変更後も表示順は変わらない(詳細設計書 0.2節)。
