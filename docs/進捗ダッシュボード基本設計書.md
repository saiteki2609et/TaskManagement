...
- 本書は [進捗ダッシュボード要件定義書](./進捗ダッシュボード要件定義書.md)(以下「要件定義書」)で確定した要件をもとに、実現方式・画面構成・データベース・処理フローを定義する基本設計書である。
...
- SCR-01〜03は既存ヘッダーの「ダッシュボード」ナビ項目([src/components/layout/header.tsx](../src/components/layout/header.tsx))を有効化し`/dashboard`に割り当てる。SCR-04/05はヘッダーの「プロジェクト」ナビ項目(`/projects`)を有効化して割り当てる(いずれも既存コード上でプレースホルダとして`active: false`かつツールチップ「近日公開予定です」で無効化済みのため、本機能実装時に有効化する)。
...
| グリッド内でステータス変更 | `updateDeliverableAction(id, { status })` | 3.2節ロジックで再集計しUI即時反映 |
| グリッド内で編集/削除 | `updateDeliverableAction` / `deleteDeliverableAction` | 削除は物理削除(`Deliverable`自体は要件定義書上ゴミ箱対象外のため確認ダイアログのみで即時削除) |
...
- 表示単位切替(週/月/四半期)は列幅・日付ラベル生成ロジックをクライアント側で切り替えるのみで、データ取得は変更しない。
- 機能・工程フィルタはクライアント側の配列フィルタで実装(取得済みデータに対して絞り込み。再フェッチ不要)。
- バークリックで`Popover`(shadcn/ui)を開き、予定日・実績日・遅延日数(`plannedEndDate`と`now`の差)を表示する。
...
├───────────────────────────────────────────────────────┤
│ 共通成果物種類マスタ(GlobalDeliverableType)               │
│ [基本設計書][詳細設計書][画面設計書][API仕様書]… [+ 追加]  │
└───────────────────────────────────────────────────────┘
```
...
| 新規作成 | `createProjectAction({ name, designDocFolderPath })` | `GlobalDeliverableType`全件を`DeliverableType`へコピー。名称重複は3.4節のDB制約違反をハンドリング |
...
| 共通マスタCRUD | `deliverable-types.ts`内の`Global*Action`群 | 既存プロジェクトの`DeliverableType`には影響しない(コピーのみで参照関係を持たない設計。6章参照) |
...
erDiagram
    ...
    GlobalDeliverableType }o..o{ DeliverableType : "作成時にコピー(参照関係なし)"
```
...
| `DesignDocument.indexStatus` / `indexedAt` | 追加 | 「未取り込み/インデックス未反映の設計書」の警告表示(要件定義書 5.3)を実現するため |
...
| `createProjectAction` | `{ name, designDocFolderPath }` | `Project` | `GlobalDeliverableType`を`DeliverableType`へ初期コピー。一意制約違反時はアプリ側エラー型で返却 |
...
| `updateDeliverableAction` | ステータス・担当者・日付・メモ等の更新。既存`updateTaskAction`同様、`status`変更時の`progress`自動補正(例: `done`→100%)を検討可(詳細設計で確定) |
| `deleteDeliverableAction` | 確認ダイアログ後に物理削除 |

### 6.5 `src/lib/actions/design-documents.ts`

| 関数 | 概要 |
| --- | --- |
| `listDesignDocumentsAction` | `projectId`配下の`DesignDocument`一覧と`indexStatus`集計(未反映件数)を返す |
| `rescanDesignDocumentsAction` | 8.2節のフォルダ走査〜embedding保存までを同期実行し、結果サマリ(`{ added, updated, skipped, failed }`)を返す |
